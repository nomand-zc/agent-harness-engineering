...
- **责任主体**：执行变更的Agent负责同步更新，Agent-6（文档熵收敛巡检Agent）负责校验一致性；
...
| `docs/slim/slim-index.md` | 规约提案统一索引，同时追踪减重与增重提案状态 | 操作人更新，Agent-6（文档熵收敛巡检Agent）校验 |
...
| 异常场景 | 判定级别 | 处置方式 |
|----------|----------|----------|
| `docs/guide/` 或 `docs/demand/` 目录不存在 | 严重 | Agent-6（文档熵收敛巡检Agent）自动预警，人类需在24小时内创建 |
