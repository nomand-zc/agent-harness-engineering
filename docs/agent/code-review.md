...
本文档定义 XX项目 的代码审查机制，作为 pre-push 自动化门禁的规范依据。
...
- **不重复检查**：golangci-lint、格式化、测试覆盖率等已由 pre-commit 其他 hook 保障，review 不重复覆盖
...
| 文档 | 说明 |
|------|------|
| [CONVENTIONS.md](CONVENTIONS.md) | 编码规范（命名、架构原则）|
| [TESTING.md](TESTING.md) | 测试规范（单元/集成/基准测试）|
