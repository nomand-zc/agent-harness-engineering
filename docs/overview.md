...
graph TD
    ...
    note2["Agent集群: Agent-0~4(执行) + 专家Agent-安全/性能/合规(评审) + Agent-5(规约指南) + Agent-6(文档熵收敛) + Workflow-Agent(质量达标后新增)"]
...
3. **Agent配置**：将所有Agent角色（Agent-0至Agent-6）配置完成，绑定对应规范文档（如Agent-0绑定《需求校准指南》，Agent-6绑定《文档熵收敛巡检检查指南》），设置触发条件（如Agent-0由人类手动触发，Agent-6由 Git pre-push 钩子自动触发）；Workflow-Agent 在各 Agent 输出质量稳定达标后再行接入，无需在初期配置；
