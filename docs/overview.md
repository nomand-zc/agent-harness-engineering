...
graph TD
    ...
    A -->|2.@Agent触发任务/深度Review产出物| F[企微群（统一交互入口）]
    F -->|3.产出不达标时@规约指南Agent修正指南| B
    F -->|4.可@单个Agent单步执行，或@Workflow-Agent全流程自动化| C[多Agent协作集群]
    B -->|提供约束/标准| C
    ...
    note2[Agent集群：Agent-0~4（执行）+ Agent-5（规约指南）+ Workflow-Agent（质量达标后新增，两种方式并存）]
    note3[产出物仓库：按需求ID分类，绑定版本与Commit]
    note4[校验体系：刚性卡点+兜底巡检+全量验收]
    note5[企微群：各Agent以数字人员工身份入群；方式A：@单个Agent单步执行；方式B：@Workflow-Agent全流程自动化；两种方式长期并存，按需选择]
...
| Agent-5（规约指南Agent） | 维护与迭代全套规约指南文档；接收人类Review反馈，修正对应指南；同时承担文档熵收敛巡检职责 | 人类Review反馈（@规约指南触发）、全链路产出物、《文档熵收敛巡检检查指南》 | 更新后的指南文档（含变更摘要）、巡检报告（含差异项、整改建议、分级预警） | 指南变更需经人类确认后方可生效；每日全量扫描+主干合并后增量扫描，不遗漏异常 |
...
> **关于 Agent-5 的双重职责说明**：Agent-5 在本方案中承担两类职责——①**规约指南迭代**：作为企微群中的「规约指南」数字人员工，接收人类对产出物的 Review 反馈，分析问题根因后修正对应指南文档，变更内容需经人类确认后方可生效，以此驱动规约持续收敛；②**文档熵收敛巡检**：每日全量扫描全链路产出物，校验文档与代码的版本联动、锚点一致性，输出分级巡检报告。两类职责共用同一 Agent 身份，前者由人类 @ 触发，后者定时自动触发。
...
3. **Agent配置**：将所有Agent角色（Agent-0至Agent-5）配置完成，绑定对应规范文档（如Agent-0绑定《需求校准指南》），设置触发条件（如Agent-0由人类手动触发）；Workflow-Agent 在各 Agent 输出质量稳定达标后再行接入，无需在初期配置；
