...
graph TD
    ...
    A -->|"2.触发任务/深度Review产出物"| F["企微群(统一交互入口)"]
    F -->|"3.产出不达标时修正指南"| B
    F -->|"4.可单步执行或全流程自动化"| C["多Agent协作集群"]
    B -->|"提供约束/标准"| C
    ...
    note2["Agent集群: Agent-0~4(执行) + 专家Agent-安全/性能/合规(评审) + Agent-5(规约指南) + Agent-6(文档熵收敛) + Agent-7(Backlog整理) + 会议助手Agent + Workflow-Agent(质量达标后新增)"]
    note3["产出物仓库: 按需求ID分类，绑定版本与Commit"]
    note4["校验体系: 刚性卡点+兜底巡检+全量验收"]
    note5["企微群: 方式A-单个Agent单步执行；方式B-Workflow-Agent全流程自动化"]
...
| Agent-3（高级开发Agent） | 对齐设计方案，编写代码、预埋单元测试；代码推送后按需触发专家Agent执行代码专项审查 | 评审通过的设计方案+验收标准、《设计方案对齐指南》；架构评审通过后自动触发 | 全量代码、单元测试用例、对齐确认文档（绑定Commit） | 代码需符合编码规范，单测通过率需达到预设阈值（如90%）；专家Agent发现致命/严重问题须整改后方可进入验收 |
| Agent-4（验收Agent） | 依据验收标准，执行全维度自动化验收测试 | Agent-3输出的代码+测试用例、验收标准文档 | 验收报告（含测试结果、不通过项、整改建议） | 验收需覆盖所有业务/接口/性能/安全场景，无遗漏 |
| 专家Agent（3类） | 补充评审，覆盖安全、性能、合规维度；在设计阶段（Agent-1输出后）和代码阶段（Agent-3推送后）均可按需触发 | Agent-1的设计方案（架构专项评审）、Agent-3的代码（代码专项审查）；人类手动@触发 | 专项评审报告（安全/性能/合规各1份，归档至 review/ 目录） | 严格遵循各专项评审规则，聚焦对应维度风险；致命/严重问题阻断后续流程 |
| Agent-5（规约指南Agent） | 维护与迭代全套规约指南文档；接收人类Review反馈，分析问题根因后修正对应指南文档，驱动规约持续收敛 | 人类Review反馈（@规约指南触发）、各Agent产出物评审结论 | 更新后的指南文档（含变更摘要、变更历史记录） | 指南变更需经人类确认后方可生效；单份指南变更频率不超过每周2次 |
| Agent-6（文档熵收敛巡检Agent） | 全链路文档熵收敛巡检；校验文档与代码的版本联动、需求ID绑定、Commit锚点一致性，输出分级巡检报告并阻断不合规推送 | Git pre-push 钩子自动触发（刚性拦截）；主干合并后增量触发；每日全量定时触发 | 巡检报告（含差异清单、整改建议、分级预警）；pre-push 阶段输出阻断/放行结论 | 严格遵循《文档熵收敛巡检检查指南》；致命/严重问题必须阻断，不可放行；不遗漏任何异常 |
| Agent-7（Backlog整理Agent） | 汇总整理各阶段产出的跟进处理事项，评估优先级，给出排期建议，将条目转化为需求草稿后通知人类确认排期，触发 Agent-0 | 需求验收通过后事件驱动；人类手动 @Backlog整理；每周一 09:00 定时触发 | 全局 Backlog 汇总（backlog.md）、整理报告、需求草稿（排期确认后） | 严格遵循《Backlog整理Agent执行指南》；P0 条目须立即通知人类，不走常规排期；不直接调用 Agent-0，由人类确认后触发 |
| Workflow-Agent（全自动化编排Agent，质量达标后新增） | 接收人类触发指令，按序编排调度 Agent-0~4 完成全流程，推送关键节点产出物至企微群；与各 Agent 数字人员工方式长期并存，使用哪种完全取决于个人 | 人类@触发指令（需求描述/需求ID）、各Agent执行结果 | 全流程执行日志、各节点产出物摘要推送 | 各Agent单次输出质量稳定达标后新增入群；任意节点异常时暂停并通知人类 |

> **关于 Agent-5 与 Agent-6 的职责拆分说明**：原方案中 Agent-5 承担双重职责，为确保职责单一、触发机制清晰，现拆分为两个独立 Agent——**Agent-5（规约指南Agent）** 专职规约指南的维护与迭代，由人类在企微群中 @ 触发，变更须经人类审批方可生效；**Agent-6（文档熵收敛巡检Agent）** 专职全链路文档熵收敛巡检，通过 Git pre-push 钩子在代码推送阶段自动触发刚性拦截，同时支持主干合并后增量扫描与每日全量定时巡检。两者职责互不交叉，触发机制独立，不存在资源竞争。
...
3. **Agent配置**：将所有Agent角色（Agent-0至Agent-7）配置完成，绑定对应规范文档（如Agent-0绑定《需求校准指南》，Agent-6绑定《文档熵收敛巡检检查指南》，Agent-7绑定《Backlog整理Agent执行指南》），设置触发条件（如Agent-0由人类手动触发，Agent-6由 Git pre-push 钩子自动触发，Agent-7支持人类手动、事件驱动、定时三种触发方式）；Workflow-Agent 在各 Agent 输出质量稳定达标后再行接入，无需在初期配置；
